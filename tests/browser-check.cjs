...
await page.locator('#btn-end').click();if(await page.locator('#leaderboard').isVisible())throw Error('duplicate results');await page.locator('#btn-results-leaderboard').click();if(!await page.locator('#leaderboard').isVisible())throw Error('missing leaderboard');if(errors.length)throw Error(errors.join());await page.evaluate(()=>navigator.serviceWorker.ready);await page.reload();await page.waitForFunction(()=>!!navigator.serviceWorker.controller);
