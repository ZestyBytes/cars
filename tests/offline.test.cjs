...
function worker({ cached, network = async () => new Response('fresh'), shell = new Response('offline html') } = {}) {
  ...
  const sandbox = { URL, Response, fetch: network, caches: { open: async () => cache, keys: async () => ['spotted-v8', 'spotted-v12', 'another-app'], delete: async key => deleted.push(key) }, self: { location: { origin: 'https://example.org' }, skipWaiting: async () => {}, clients: { claim: async () => {} }, addEventListener: (name, fn) => handlers[name] = fn } };
